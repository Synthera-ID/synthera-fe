...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-5 relative overflow-hidden font-sans bg-bg-1">
      ...
      {/* Card Login */}
      <div className="w-full max-w-[380px] rounded-2xl p-8 z-10 relative" style={{ background: 'rgba(24, 20, 43, 0.88)', border: '1px solid rgba(139,92,246,0.2)', boxShadow: '0 8px 40px rgba(0,0,0,0.6)', backdropFilter: 'blur(20px)' }}>
        ...
        <form className="flex flex-col gap-3" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="flex justify-between items-center text-[12px]">
            ...
            <Link href="/forgot-password" className="text-[#94A3B8] hover:text-violet-400 transition-colors">Forgot password?</Link>
          </div>
        ...
        <div className="flex flex-col gap-2.5">
          <Button onClick={RedirectGoogle} type="button" variant="glass" className="w-full py-2.5 text-[13px] rounded-lg border-white/10 flex items-center justify-center gap-2.5">
            <FcGoogle size={15} /> Continue with Google
          </Button>
          <Button type="button" variant="glass" className="w-full py-2.5 text-[13px] rounded-lg border-white/10 flex items-center justify-center gap-2.5">
        ...
        <p className="text-center mt-5 text-[12px] text-[#94A3B8]">
          Don't have an account? <Link href="/register" className="text-violet-400 hover:text-violet-300 transition-colors">Register</Link>
        </p>
