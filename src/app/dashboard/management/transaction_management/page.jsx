...
export default function TransactionManagementPage() {
  ...
  function handleExportExcel() {
    ...
    const header = ["Invoice", "Email", "Plan", "Amount (IDR)", "Status", "Date", "Payment Method"];
    const rows = transactions.map((t) => [
      t.invoice_code ?? "-",
      t.user?.email ?? "-",
      t.plan?.name ?? "-",
      Number(t.final_amount ?? t.amount ?? 0),
    ...
    openExcelPreview({
      header,
      rows,
      filename: `transactions_${filterStatus}_${new Date().toISOString().slice(0, 10)}.xlsx`,
      documentTitle: `Transaction Management`,
    });
  }
