...
const TRANSACTIONS = [
  ...
  { id: "INV-2025-0013", date: "Dec 15, 2025", description: "Pro Plan - Monthly", amount: "$29.00", status: "paid" },
  {
    id: "INV-2025-0012",
    date: "Nov 15, 2025",
    description: "Plan Upgrade (Starter→Pro)",
    amount: "$29.00",
    status: "paid",
  },
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              ...
              <ChevronDown
                size={14}
                className={`text-text-3 transition-transform ${dropdownOpen ? "rotate-180" : ""}`}
              />
            </button>
            {dropdownOpen && (
              <div className="absolute top-full mt-1 right-0 w-full bg-bg-2 border border-bg-3 rounded-xl shadow-xl shadow-black/20 z-50 overflow-hidden">
                {["all", "paid", "failed"].map((s) => (
                  <button
                    ...
                    className={`w-full px-4 py-2.5 text-left text-[13px] capitalize transition-colors ${
                      statusFilter === s
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((t, i) => {
            ...
            return (
              <div
                key={t.id}
                className={`grid grid-cols-[1.1fr_1fr_1.6fr_0.9fr_0.7fr_0.5fr] gap-4 px-6 py-4 items-center hover:bg-bg-3/30 transition-colors duration-150 ${
              ...
              >
                ...
                <div>
                  <span
                    className={`inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold border ${cfg.bg} ${cfg.border} ${cfg.color}`}
                  >
                    {cfg.label}
                  </span>
                </div>
