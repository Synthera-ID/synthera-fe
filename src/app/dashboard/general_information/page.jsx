...
const CHANGELOG = [
  ...
  { version: "v3.4.0", date: "Apr 10, 2026", type: "minor", notes: "Added webhook retry mechanism and improved latency on Content Delivery." },
  { version: "v3.3.0", date: "Mar 18, 2026", type: "minor", notes: "Introduced User Management API endpoints and bulk operations." },
  { version: "v3.2.5", date: "Feb 28, 2026", type: "patch", notes: "Security patch for JWT refresh token rotation." },
];

const STATUS_STYLES = {
  Operational: { dot: "bg-emerald-400", text: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
      ...
      <div className="flex-1 flex flex-col min-h-screen min-w-0">
        ...
        <main className="flex-1 p-8 lg:p-12 overflow-y-auto w-full scroll-smooth">
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
            <StatCard icon={<Globe size={20} className="text-violet-400" />}   iconBg="bg-violet-500/20"  label="API Version"      value="v3.4.1" />
            <StatCard icon={<Server size={20} className="text-blue-400" />}    iconBg="bg-blue-500/20"    label="Services Online"  value={`${operationalCount}/${SERVICE_STATUS.length}`} />
            <StatCard icon={<Shield size={20} className="text-emerald-400" />} iconBg="bg-emerald-500/20" label="Security Status"  value="Secure" />
            <StatCard icon={<Clock size={20} className="text-amber-400" />}    iconBg="bg-amber-500/20"   label="Avg Latency"      value="52ms" />
          </div>
                  ...
                  return (
                    <div key={name} className="flex items-center justify-between px-6 py-3.5 gap-3">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <span className={`text-[11px] font-bold px-2.5 py-1 rounded-md border ${s.bg} ${s.text} ${s.border}`}>
                          {status}
                        </span>
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-6">
            <InfoTile icon={<Lock size={18} className="text-emerald-400" />} iconBg="bg-emerald-500/15" title="TLS 1.3 Encryption" desc="All data in transit is encrypted with TLS 1.3. Certificates auto-renewed via Let's Encrypt." />
            <InfoTile icon={<Shield size={18} className="text-violet-400" />} iconBg="bg-violet-500/15" title="SOC 2 Type II" desc="The platform is SOC 2 Type II certified. Annual security audits are conducted by a third-party." />
            <InfoTile icon={<Cpu size={18} className="text-blue-400" />} iconBg="bg-blue-500/15" title="Auto Scaling" desc="Infrastructure scales automatically based on demand. Zero downtime deployments are guaranteed." />
          </div>
              ...
              {CHANGELOG.map(({ version, date, type, notes }) => {
                ...
                return (
                  <div key={version} className="flex items-start gap-4 px-6 py-4 hover:bg-bg-3/20 transition-colors">
                    <span className={`mt-0.5 inline-block px-2.5 py-1 rounded-md text-[11px] font-bold border shrink-0 ${t.bg} ${t.text} ${t.border}`}>
                      {t.label}
                    </span>
